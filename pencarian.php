...
FROM sub_kriteria ORDER BY range_atas, range_bawah DESC  ");
...
<!-- <div class="row gy-4 text-white">
      ...
      <?php if(isset($_POST['submit'])): ?>
        <div class="col-12 text-dark text-center">
          <div class="section-header">
                  <h2 class="">Hasil Survey ada <?= count($result)?> motor </h2>
          </div>
        </div>
          <div class="table-responsive">
            ...
            ">
              <thead class="table-light">
                <tr>
                  <th>Jumlah Motor yang disurvey</th>
                  <td> <?= count($result)?></td>
                </tr>
                <tr>
                  <th>Jumlah Kriteria yang dipilih</th>
                  <td> <?= $j ?></td>
                </tr>

                <tr>
                  <th>Peluang Terpilih</th>
                  <td> <?php $terpilih = round($j/count($result),2); echo $j .' / '. count($result).'='. $terpilih ?> </td>
                </tr>
                <tr>
                  <th>Peluang Tidak Terpilih</th>
                  <td> <?php $tidak = round(1 - $terpilih,2) ; echo 1 .' - '. $terpilih .'='. $tidak ?> </td>
          ...
          <div class="section-header">
                  <h2 class="">Daftar Motor Hasil Survey </h2>
          </div>
          <div class="table-responsive">
            <table class="table table-striped
            table-hover	
                ...
                <tbody class="table-group-divider">
                  <?php  $i=0; foreach($result as $data):?>
                    <tr class="" >
                      ...
                      <td><?= $data['hasil']?></td>
                      <!-- <td>Rp. <?= number_format($data['Harga_Motor_atas'],0,',','.') ?></td> -->
                      <td>  <?php
          if ($_SESSION != null ){
            if ($_SESSION['role'] == '3') {
                echo '
                <form action="keranjang" class="d-inline-flex" method="POST"> 
                  ...
                  <button type="submit" class="btn btn-success m-2" name="submit">  <i class="bi bi-bag-fill"></i> Pesan </button>
                </form>
                ';
              }else{
                echo ' <button data-bs-toggle="modal" data-bs-target="#role" class="btn btn-success m-2">  <i class="bi bi-bag-fill"></i> Pesan </button>';
              }
            ...
            <form action="detail.php" method="post" class="d-inline">
              ...
              <button type="submit"  class="btn btn-primary m-2">  <i class="bi bi-eye-fill"></i> Detail </button>
          ...
          <?php
          // Fungsi pembanding untuk mengurutkan berdasarkan nilai
              function compareByHarga($a, $b) {
                return $a['Harga_Motor_atas'] - $b['Harga_Motor_atas'];
              }

              // Mengurutkan array dua dimensi berdasarkan nilai dari terkecil ke terbesar
              // usort($result, 'compareByHarga');
            ...
            <div class="col-lg-4 col-md-6 mb-4" data-aos="fade-up" data-aos-delay="100">
              <div class="card">
                ...
                <div class="p-3"> 
                  ...
                  <h5 class="text-dark">   Rp. <?php if($data['Harga_Motor_atas'] != null) echo number_format($data['Harga_Motor_atas'],0,',','.')  ?>  </h5>
 ...
 <form action="" method="post">
  ...
  <div class="row container mt-5">
   ...
   <div class="col-12">
    <div class="mb-3">
      <label for="" class="form-label">Jumlah Motor Yang Di Survey</label>
      <input type="text"
        class="form-control bg-light" name="" readonly  id="" value="<?= $motor->num_rows?>" aria-describedby="helpId" placeholder="">
    </div>
   </div>
